...
4. **推送私聊语音** → 用户（从 `rpctvm_targets.json` 读取 `user_open_id`，最近24小时数据）
   ...
   - **格式要求**: 简短但突出重点
     - 开头: "📊 浦发隧道设备日报，{日期}"
     - 正文: 优先播报特殊/失联设备，然后是一般关注设备
     - **重点突出**: 失联设备或特殊关注设备要加重语气
       - 特殊设备: "⚠️ 特殊关注：{设备名}，{问题}"
       - 失联设备: "🚨 失联设备：{设备名}，请立即检查！"
       - 一般设备: "{设备名}，{问题}"
     - 结尾: "共{X}台设备需关注，详情见群消息"
     - 示例: "📊 浦发隧道设备日报，3月16日。⚠️ 特殊关注：PuFaJiTuan-010000-01，Wan2双链路异常。PuFaJiTuan-100200-01，Wan2能见度80%。共2台设备需关注，详情见群消息。"
5. **写入多维表格** → 排除当天，仅新日期
